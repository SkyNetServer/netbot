...
module.exports = class LaunchCybernukeCommand extends Command {
    ...
    async run(msg, { age, join }) {
        const statusMsg = await msg.say("Calculating targeting parameters for cybernuke...");
        ...
        while (!statusMsg2) {
            ...
            if (!responses || responses.size !== 1) {
                await msg.say("Cybernuke cancelled.");
                return null;
            }
        ...
        for (const member of members.values()) {
            promises.push(
				...
				`).catch(err => this.client.logger.cybernuke(err.stack))
                    .then(() => member.ban())
                    .then(() => {
                        fatalities.push(member);
                    })
                    .catch(err => {
                        this.client.logger.cybernuke(err.stack);
                        survivors.push({
                            member: member.id,
                            error: err
                        });
